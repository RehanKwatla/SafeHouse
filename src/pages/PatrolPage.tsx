    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-3xs mono text-ink-faint">WAYPOINT NAVIGATION · AUTOMATED ROOM SWEEP</span>
        </div>
        <SimulationToggle />
      </div>
        ...
        <div className="panel-header bg-base-elevated">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className={`status-dot ${isRunning ? 'bg-green animate-pulse-green' : isPaused ? 'bg-amber' : 'bg-ink-faint'}`} />
          ...
          <span className="text-3xs mono text-ink-faint">STARTED {formatTime(patrol.startedAt)}</span>
        </div>
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-xs mono text-ink font-semibold tabular-nums">{Math.round(patrol.progress)}%</span>
            </div>
            ...
            {patrol.rooms.map((roomId) => {
              ...
              return (
                <div
                  ...
                  style={{ borderLeft: `2px solid ${done ? '#A8F04D' : current ? '#A8F04D' : '#1C292D'}` }}
                >
                  <div className="flex items-center gap-2 mb-1">
                    {done    ? <CheckCircle2 className="w-3.5 h-3.5 text-green" /> :
                     current ? <ArrowRight   className="w-3.5 h-3.5 text-green" /> :
                               <Circle       className="w-3.5 h-3.5 text-ink-faint" />}
                    <span className="text-2xs mono font-bold text-ink">{formatRoom(roomId)}</span>
                  </div>
                  <p className={`text-3xs mono tracking-widest ${done ? 'text-green' : current ? 'text-green' : 'text-ink-faint'}`}>
      ...
      <div className="panel" style={{ borderTop: '2px solid #263540' }}>
        ...
        <div className="px-4 py-3 grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            ...(robot.targetRoom !== null ? [{ label: 'TARGET', value: formatRoom(robot.targetRoom), color: 'text-ink' }] : []),
            ...(robot.etaSeconds > 0     ? [{ label: 'ETA',    value: `${robot.etaSeconds}s`,       color: 'text-green' }] : []),
