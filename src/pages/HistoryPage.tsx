...
import { Thermometer, Droplets, AudioLines, AlertTriangle, Map as MapIcon } from 'lucide-react';
...
function ChartTooltip({ active, payload, label, unit }: {
  ...
  return (
    <div className="panel-elevated px-2.5 py-1.5">
      ...
      <p className="text-xs mono text-ink">{payload[0].value.toFixed(1)}{unit}</p>
    </div>
  );
}
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-3xs mono text-ink-faint">SENSOR TRENDS · PATROL LOG · ALERT LOG</span>
        </div>
          ...
          <div className="px-4 py-3">
            ...
            <ResponsiveContainer width="100%" height={120}>
              <LineChart data={chartData} margin={{ top: 2, right: 4, bottom: 2, left: -24 }}>
                ...
                <XAxis dataKey="time" tick={{ fill: CHART_PROPS.tickFill, fontSize: 8, fontFamily: 'monospace' }}
                  axisLine={{ stroke: CHART_PROPS.axisStroke }} tickLine={false} interval="preserveStartEnd" />
                <YAxis tick={{ fill: CHART_PROPS.tickFill, fontSize: 8, fontFamily: 'monospace' }}
                  axisLine={{ stroke: CHART_PROPS.axisStroke }} tickLine={false} domain={[10, 35]} />
                <Tooltip content={<ChartTooltip unit="°C" />} />
                <Line type="monotone" dataKey="temperature" stroke="#F2B84B" strokeWidth={1.5} dot={false} animationDuration={300} />
              </LineChart>
            </ResponsiveContainer>
          </div>
            ...
            <ResponsiveContainer width="100%" height={120}>
              <LineChart data={chartData} margin={{ top: 2, right: 4, bottom: 2, left: -24 }}>
                ...
                <XAxis dataKey="time" tick={{ fill: CHART_PROPS.tickFill, fontSize: 8, fontFamily: 'monospace' }}
                  axisLine={{ stroke: CHART_PROPS.axisStroke }} tickLine={false} interval="preserveStartEnd" />
                <YAxis tick={{ fill: CHART_PROPS.tickFill, fontSize: 8, fontFamily: 'monospace' }}
                  axisLine={{ stroke: CHART_PROPS.axisStroke }} tickLine={false} domain={[20, 100]} />
                <Tooltip content={<ChartTooltip unit="%" />} />
                <Line type="monotone" dataKey="humidity" stroke="#55D6E8" strokeWidth={1.5} dot={false} animationDuration={300} />
              </LineChart>
            </ResponsiveContainer>
          </div>
            ...
            <ResponsiveContainer width="100%" height={120}>
              <LineChart data={chartData} margin={{ top: 2, right: 4, bottom: 2, left: -24 }}>
                ...
                <XAxis dataKey="time" tick={{ fill: CHART_PROPS.tickFill, fontSize: 8, fontFamily: 'monospace' }}
                  axisLine={{ stroke: CHART_PROPS.axisStroke }} tickLine={false} interval="preserveStartEnd" />
                <YAxis tick={{ fill: CHART_PROPS.tickFill, fontSize: 8, fontFamily: 'monospace' }}
                  axisLine={{ stroke: CHART_PROPS.axisStroke }} tickLine={false} domain={[0, 100]} />
                <Tooltip content={<ChartTooltip unit=" dB" />} />
                <Line type="monotone" dataKey="sound" stroke="#758287" strokeWidth={1.5} dot={false} animationDuration={300} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <div className="divide-y divide-line-faint max-h-72 overflow-y-auto scrollbar-thin">
              {records.map((rec) => (
                <div key={rec.id} className="flex items-center gap-3 px-4 py-2.5">
                  <span className={`status-dot ${rec.status === 'COMPLETED' ? 'bg-green' : rec.status === 'WARNING' ? 'bg-amber' : 'bg-red'}`} />
                  <span className="text-2xs mono text-ink-faint w-14">{formatTime(rec.timestamp)}</span>
                  <span className="text-2xs mono text-ink">MISSION #{String(rec.id).padStart(3,'0')}</span>
            ...
            <div className="divide-y divide-line-faint max-h-72 overflow-y-auto scrollbar-thin">
              {alerts.slice(0, 20).map((alert) => (
                <div key={alert.id} className="flex items-center gap-3 px-4 py-2.5"
                  style={{ borderLeft: `2px solid ${alert.severity === 'critical' ? '#FF4D4D' : alert.severity === 'warning' ? '#F2B84B' : '#263540'}` }}>
                  <span className="text-2xs mono text-ink-faint w-14">{formatTime(alert.timestamp)}</span>
                  <span className="text-2xs mono text-cyan w-14">{formatRoom(alert.room)}</span>
                  <span className="text-xs text-ink-muted flex-1 truncate">{alert.description}</span>
                  <span className={`text-3xs mono ${alert.state === 'ACTIVE' ? 'text-red' : 'text-green'}`}>
                    {alert.state}
                  </span>
                </div>
              ))}
